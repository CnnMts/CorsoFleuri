...
class DiscountModel extends SqlConnect {
  private $table = "discount";
  public $authorized_fields_to_update = ['name', 'percent'];
  ...
  public function add(array $data) {
    ...
    if ($req->rowCount() > 0) {
      throw new HttpException("Discount already exists!", 400);
    }

    $query = "
      INSERT INTO $this->table (name, percent)
      VALUES (:name, :percent)
    ";
  ...
  public function get(int $id) {
    ...
    if ($req->rowCount() == 0) {
      throw new HttpException("Discount doesn't exists !", 400);
    }
  ...
  public function getAll(?int $limit = null) {
    ...
    if ($req->rowCount() == 0) {
      throw new HttpException("No Discounts !", 400);
    }
